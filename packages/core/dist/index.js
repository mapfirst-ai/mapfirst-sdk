...
__export(index_exports, {
  ...
  LeafletMarkerManager: () => LeafletMarkerManager,
...
var LOADING_SPINNER_HTML = `<span class="mapfirst-marker-loading-spinner"></span>`;
...
function createPrimaryMarkerElement(item, primaryType, selectedMarkerId, onMarkerClick, markerOptions) {
  ...
  const selectedAward = ((_c = marker.awards) == null ? void 0 : _c.length) ? [...marker.awards].sort((a, b) => {
    ...
    if (((_a2 = b.year) != null ? _a2 : 0) !== ((_b2 = a.year) != null ? _b2 : 0)) {
      return ((_c2 = b.year) != null ? _c2 : 0) - ((_d2 = a.year) != null ? _d2 : 0);
    }
  ...
  if ((markerOptions == null ? void 0 : markerOptions.showTail) && isPrimaryType) {
  ...
  if (!isPending && !(markerOptions == null ? void 0 : markerOptions.hideBadge)) {
    if ((markerOptions == null ? void 0 : markerOptions.badgeImageUrl) && isAccommodation) {
      ...
      if (selectedAward == null ? void 0 : selectedAward.type) {
        ...
        const awardTypeKey = selectedAward.type === "Certificate of Excellence" ? "0" : "1";
  ...
  if ((markerOptions == null ? void 0 : markerOptions.showLabel) && isPrimaryType && !isPending) {
...
// src/dotmarker.ts
function createDotMarkerElement(item, primaryType, selectedMarkerId, onMarkerClick, markerOptions) {
  var _a, _b, _c;
  if (typeof document === "undefined") {
    return null;
  }
...
var BaseMarkerManager = class {
  ...
  render(items, primaryType, selectedMarkerId) {
    ...
    const effectiveItems = ((_a = this.markerOptions) == null ? void 0 : _a.hideNonPrimary) ? items.filter((item) => item.marker.type === this.primaryType) : items;
...
var MapLibreMarkerManager = class extends BaseMapGLMarkerManager {
   ...
   */
  hideOverlappingLabels(items) {
    const map = this.mapInstance;
    const markerBoxes = [];
    const labelInfo = [];
    for (const item of items) {
      const loc = item.marker.location;
      if (typeof (loc == null ? void 0 : loc.lon) !== "number" || typeof (loc == null ? void 0 : loc.lat) !== "number") continue;
      let pt;
      try {
      ...
      if (item.kind === "primary") {
        ...
        if (item.marker.type === this.primaryType) {
          ...
          const label = root == null ? void 0 : root.querySelector(
            ".mapfirst-marker-label"
          );
          if (label) {
...
function clusterMarkers({
  ...
  for (let i = 0; i < projected.length; i += 1) {
    for (let j = i + 1; j < projected.length; j += 1) {
      ...
      const collides = labelExtentPx !== void 0 ? Math.abs(dx) <= threshold + labelExtentPx && Math.abs(dy) <= threshold : Math.hypot(dx, dy) <= threshold;
      if (collides) {
        union(i, j);
      }
    }
...
var MapFirstCore = class {
  ...
  async streamPricing({
    ...
    try {
      ...
      if (isCancelled == null ? void 0 : isCancelled()) return { completed };
      (_c = (_b = this.callbacks).onPropertiesLoadError) == null ? void 0 : _c.call(_b, error);
      return { completed };
    } finally {
      controller.abort();
    }
  ...
  applyPricingBatch(success, price) {
    var _a, _b, _c, _d, _e;
    const results = (_a = success == null ? void 0 : success.results) != null ? _a : [];
    const unsupportedIds = new Set((_c = (_b = success == null ? void 0 : success.invalidHotelIds) == null ? void 0 : _b.map(Number)) != null ? _c : []);
    const unsupportedIds2 = new Set(
      (_e = (_d = success == null ? void 0 : success.unsupportedHotelIds) == null ? void 0 : _d.map(Number)) != null ? _e : []
    );
  ...
  async runSmartFilterSearch({
    ...
    const { primary_type: _basePrimaryType, ...baseFilterPayload } = this.getFilters();
    let filterPayload = baseFilterPayload;
  ...
  refresh() {
    ...
    (_d = (_c = this.options).onClusterUpdate) == null ? void 0 : _d.call(_c, this.clusterItems, viewState);
  }
  destroy() {
    var _a;
    if (this.destroyed) {
